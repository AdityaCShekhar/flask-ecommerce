...
from services.auth_service import token_required  # Import token_required decorator

products = Blueprint("products", __name__)

@products.route("/add_product", methods=["POST"])
@token_required
def add_product_route(current_user):  # JWT-protected
...
@token_required
def get_product_by_id_route(current_user, product_id):  # JWT-protected
    response, status_code = get_product_by_id(product_id)
    return jsonify(response), status_code

@products.route("/")
@token_required
def get_products(current_user):  # JWT-protected
